...
interface TafsirAudioCardProps {
  audio: TafsirAudio;
  isPlaying: boolean;
  onClick: () => void;
  index: number;
...
export function TafsirAudioCard({ audio, isPlaying, onClick, index }: TafsirAudioCardProps) {
  return (
    <motion.div
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${isPlaying ? 'bg-primary dark:bg-accent-light' : 'bg-primary/10 dark:bg-primary/20'}`}>
            <Mic2 className={`transition-colors ${isPlaying ? 'text-white dark:text-space-300' : 'text-primary dark:text-primary-light'}`} size={28} />
          </div>
